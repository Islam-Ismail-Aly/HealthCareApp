...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final DatabaseReference myref = fDatabase.getReference("StepCounter").child(user.getUid());
        final DatabaseReference myref1 = fDatabase.getReference(user.getUid()).child("push");

        Tacker.setOnIncrementListener(new Runnable() {
            @Override
            public void run() {
                stepvalue = String.valueOf(Tacker.getValue());
                myref.push().setValue("stepcounter =" + stepvalue);
                myref1.push().setValue("push =" + stepvalue);
            }
        });
